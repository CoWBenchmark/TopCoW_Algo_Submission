...
If you use this template you can simply replace the `your_segmentation_algorithm` function in your_algorithm.py with your own algorithm. 
...
import numpy as np
from your_algorithm import TRACK
from your_algorithm import your_segmentation_algorithm
...
def write_array_as_image_file(*, array, input_folder, output_folder):
    ...
    # NOTE: Your output prediction must have the same shape as the input image of the chosen track!
    ...
    assert array.shape == required_output_shape, "Prediction output must have the same shape as the input image!"
